...
class level4: SKScene, SKPhysicsContactDelegate {
    ...
    override func sceneDidLoad() {
        ...
        if HomeScene.level4HaveStar1 != nil{
        haveStar1 = HomeScene.level4HaveStar1
        }
        if HomeScene.level4HaveStar2 != nil{
        haveStar2 = HomeScene.level4HaveStar2
        }
        if HomeScene.level4HaveStar3 != nil{
        haveStar3 = HomeScene.level4HaveStar3
        }
        ...
        let moveMoveBlockRight = SKAction.moveTo(y: 462, duration: 4)
        let moveMoveBlockLeft = SKAction.moveTo(y: 142, duration: 4)
...
if fourGrid!.contains(floor4!.position){
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        for t in touches {
            ...
            nextLevelButtonShade!.run(resetButtonShrink2)
                ...
                let changeScene = SKAction.run {
                let mainmenuscene = level5(fileNamed: "level5")
                mainmenuscene?.scaleMode = .aspectFill
                self.scene?.view?.presentScene(mainmenuscene!)
                    //, transition: SKTransition.fade(with: .black, duration: 3.0))
                }
    ...
    func bulletHitEmitter(position: CGPoint) {
        
        let name = SKEmitterNode(fileNamed: "bulletHit.sks")
    ...
    func positions(){
        ...
        cameraNode.position.y = player!.position.y + 120
            ...
            let cameraYMove = SKAction.move(to: CGPoint(x: player!.position.x + 200, y: ground!.position.y + 1400), duration: 2)
        ...
        if haveStar2 == false || haveStar2 == nil{
            ...
            Star2!.position = CGPoint(x: 1240, y: 680)
        
            StarShade2!.position = CGPoint(x: 1240, y: 672)
        ...
        if haveStar3 == false || haveStar3 == nil{
            ...
            Star3!.position = CGPoint(x: 2320, y: 880)
        
            StarShade3!.position = CGPoint(x: 2320, y: 872)
    ...
    func resetfunc(){
        ...
        let runone = SKAction.run {
            ...
            if level4.haveStar12 == false || level4.haveStar12 == nil{
            if HomeScene.level4HaveStar1 != true{
    ...
    //MARK: Bullet
    func addBullet(Gun: SKSpriteNode, GunBase: SKSpriteNode) {
        
    let anglelocBack = CGPoint(x: Gun.position.x + (40 * cos(Gun.zRotation + (3.14) / 2)), y: Gun.position.y + (50 * sin(Gun.zRotation + (3.14) / 2)))
                
    let anglelocStart = CGPoint(x: Gun.position.x + (50 * cos(Gun.zRotation + (3 * 3.14) / 2)), y: Gun.position.y + (50 * sin(Gun.zRotation + (3 * 3.14) / 2)))
        
    let anglelocFront = CGPoint(x: Gun.position.x + (70 * cos(Gun.zRotation + (3 * 3.14) / 2)), y: Gun.position.y + (70 * sin(Gun.zRotation + (3 * 3.14) / 2)))
        
    bullet.name = "bullet"
    bullet = SKSpriteNode(imageNamed: "RedPlayer")
    bullet.position = anglelocStart
    bullet.zPosition = -11
    bullet.size = CGSize(width: 13, height: 23)
    bullet.zRotation = Gun.zRotation
    let offset = anglelocFront - bullet.position
    let direction = offset.normalized()
    let shootAmount = direction * 450
    let realDest = shootAmount + bullet.position
    let actionMove = SKAction.move(to: realDest, duration: 1.0)
    let recoilBack = SKAction.move(to: anglelocBack, duration: 0.1)
        let recoilForward = SKAction.move(to: GunBase.position, duration: 0.8)
    let gunRecoil = SKAction.sequence([recoilBack,recoilForward])
    //let actionMoveDone = SKAction.removeFromParent()
    //let bulletFO = SKAction.fadeOut(withDuration: 0.2)
    ...
    addChild(bullet)
       // bullet!.physicsBody?.applyImpulse(CGVector(dx: anglelocBack.x - Gun.position.x, dy: anglelocBack.y - Gun.position.y))
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if player!.contains(bullet.position){
            ...
            if player!.contains(bullet.position) || realground!.contains(bullet.position){
                        
                        bulletHitEmitter(position: bullet.position)
                bulletHitEmitter(position: bullet.position)
        ...
        if finishLine!.contains(player!.position) {
            ...
            HomeScene.level4Completed = true
            passedFinishLine2 = true
            
            level4.haveStar12 = haveStar1
            level4.haveStar22 = haveStar2
            level4.haveStar32 = haveStar3
